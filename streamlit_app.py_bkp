...
with st.sidebar:
    ...
    selected = st.radio("", [p[0] for p in PAGES], label_visibility="collapsed")
...
@st.cache_data(ttl=30, show_spinner=False)
def load_runs():
    return query("SELECT * FROM ml_features ORDER BY run_id DESC")

@st.cache_data(ttl=30, show_spinner=False)
def load_tax():
    return query("""
        SELECT ots.*, ots.orchestration_tax_uj/1e6 AS tax_j,
...
# ═════════════════════════════════════════════════════════════
    ...
    if not exps.empty:
        ...
        exp_tax  = query(f"""
            SELECT ots.*, ots.orchestration_tax_uj/1e6 AS tax_j, ots.tax_percent
            FROM orchestration_tax_summary ots
