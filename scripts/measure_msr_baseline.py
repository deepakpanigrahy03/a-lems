...
def measure_baseline(config):
    ...
    iterations = config.get('settings', {}).get('msr', {}).get('wakeup_latency_iterations', 500)
...
def main():
    ...
    parser.add_argument('--force', action='store_true', help='Remeasure even if baseline exists')
    args = parser.parse_args()

    if not args.force and BASELINE_FILE.exists():
        logger.info(f"Baseline already exists at {BASELINE_FILE}. Use --force to remeasure.")
        return
