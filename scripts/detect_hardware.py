...
def detect_tsc_frequency() -> Optional[int]:
    ...
    try:
        ...
        cmd = ["turbostat", "--quiet", "--show", "TSC_MHz", "--interval", "1", "sleep", "1"]
...
def detect_ring_bus_limits() -> Dict[str, Any]:
    ...
    try:
        ...
        result = subprocess.run(['rdmsr', '0x621'], capture_output=True, text=True, timeout=1)
        if result.returncode == 0:
            ...
            print(f"   ✅ Ring bus limits from MSR: min={limits['min_mhz']} MHz, max={limits['max_mhz']} MHz")
            return limits
    except Exception as e:
...
def detect_ring_bus_base_clock() -> float:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"   ✅ MSR_PLATFORM_INFO accessible - using default base clock {base_clock} MHz")
        else:
            print(f"   ℹ️  MSR_PLATFORM_INFO not accessible - using default base clock {base_clock} MHz")
    except Exception:
        print(f"   ℹ️  MSR access not available - using default base clock {base_clock} MHz")
    ...
    if args.output:
        ...
        if args.merge and output_path.exists():
            ...
            try:
                ...
                if 'tsc_frequency_hz' in new_cpu:
                    ...
                    existing['cpu']['tsc_detection_method'] = new_cpu.get('tsc_detection_method', 'auto_detected')
                # ============================================================
        ...
        if args.verbose:
            ...
            if 'ring_bus_base_clock_mhz' in msr_config:
                print(f"   Ring bus base clock: {msr_config['ring_bus_base_clock_mhz']} MHz")
