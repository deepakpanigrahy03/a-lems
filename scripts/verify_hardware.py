...
def check_msr(config):
    ...
    rdmsr_path = subprocess.run(["which", "rdmsr"], capture_output=True, text=True).stdout.strip()
    if not rdmsr_path:
    ...
    if 'cap_sys_rawio=ep' not in cap_result.stdout:
        print_status("  rdmsr capabilities", "⚠️", "Missing. Run: sudo setcap cap_sys_rawio=ep $(which rdmsr)")
    
    # Try to read a simple MSR (IA32_PLATFORM_ID = 0x10)
    # If this succeeds, MSR access is working
    try:
        result = subprocess.run(["rdmsr", "0x10"], capture_output=True, text=True, timeout=1)
...
def check_msr_config(config):
    ...
    if cstate_max:
        ...
        if cstate_max == expected_max:
            ...
            print_status("  cstate_counter_max", "⚠️", f"{cstate_max} (expected {expected_max})")
            all_ok = False
    else:
    ...
    if base_clock:
        if base_clock == 100:
            ...
            print_status("  ring_bus_base_clock_mhz", "⚠️", f"{base_clock} MHz (typical is 100 MHz)")
    ...
    if wakeup_idle:
        if 1 <= wakeup_idle <= 5:
            ...
            print_status("  wakeup_idle_ms", "⚠️", f"{wakeup_idle} ms (typical range 1-5 ms)")
    else:
...
def check_tsc_frequency(config):
    ...
    if tsc_hz:
        ...
        print_status("  TSC frequency", "✅", f"{tsc_mhz:.0f} MHz (detected: {detection_method})")
        return True
    else:
        print_status("  TSC frequency", "⚠️", "Not detected - MSRReader will use fallback")
        return False
...
def check_turbostat(config):
    ...
    # ------------------------------------------------------------------------
    try:
        result = subprocess.run(["turbostat", "--version"], capture_output=True, text=True, timeout=1)
        if result.returncode == 0:
            print_status("  turbostat command", "✅", "Works")
            
            # Quick test with --no-msr to avoid permission issues
            test_cmd = ["turbostat", "--quiet", "--no-msr", "--show", "all", "sleep", "0.1"]
...
def main():
    ...
    passed = sum([rapl_ok, msr_ok, msr_config_ok, ring_bus_ok, tsc_ok,
                  turbostat_ok, thermal_ok, cpufreq_ok])
