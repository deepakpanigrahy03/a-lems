...
from core.sustainability.utils import EnergyConverter, UnitConverter, GWPCalculator, GridFactorManager
...
class SustainabilityResult:
    ...
    def summary(self) -> str:
        ...
        lines = [
            ...
            f"  • {self.carbon.per_query_mg:.6f} mg/query  [Req 2.9]" if self.carbon.per_query_mg else "",
            f"  • Source: {self.carbon.source} ({self.carbon.year})  [Req 2.1]",
            "",
            f"Water: {self.water.liters*1000:.6f} ml  [Req 2.5]",
            f"  • {self.water.per_query_ml:.6f} ml/query  [Req 2.10]" if self.water.per_query_ml else "",
            f"  • Source: {self.water.source} ({self.water.year})  [Req 2.2]",
            "",
            f"Methane: {self.methane.grams:.6f} g CH₄  [Req 2.6]",
            f"  • CO₂e (20yr): {self.methane.co2e_20yr*1000:.6f} g" if self.methane.co2e_20yr else "",
            f"  • CO₂e (100yr): {self.methane.co2e_100yr*1000:.6f} g" if self.methane.co2e_100yr else "",
...
class SustainabilityCalculator:
    ...
    def calculate_from_derived(self,
    ...
    def calculate_arbitrage(self, 
                           energy_kwh: float, 
                           current_country: str = "US") -> Dict[str, Any]:
        """
    ...
    # Req 2.13: Energy Scarcity Index (ESI)
    # ========================================================================
    def calculate_scarcity_index(self,
                                energy_kwh: float,
                                country_code: str = "US") -> float:
        """
...
if __name__ == "__main__":
    ...
    if args.ip_detect or (args.country is None and args.ip_detect):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"   Geolocation API returned {resp.status_code}, falling back to US.")
    ...
    result_raw = calculator.calculate_from_raw(raw, country_code=country_code, query_count=1)
    print("\n📊 From Raw (Total Energy):")
    print(result_raw.summary())

    result_derived = calculator.calculate_from_derived(derived, country_code=country_code, query_count=1)
    ...
    arbitrage = calculator.calculate_arbitrage(derived.workload_energy_j / 3.6e6, country_code)
    print(f"   Current ({country_code}): {arbitrage['current_carbon_kg']:.6f} kg CO₂e")
    print(f"   Best ({arbitrage['best_country']}): {arbitrage['best_carbon_kg']:.6f} kg CO₂e")
    print(f"   Potential savings: {arbitrage['potential_savings_percent']:.6f}%")

    print("\n📊 Energy Scarcity Index [Req 2.13]:")
    esi = calculator.calculate_scarcity_index(derived.workload_energy_j / 3.6e6, country_code)
