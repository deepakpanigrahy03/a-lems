...
class GridFactorManager:
            ...
            for country_code, data in grid_data.items():
                ...
                if carbon is not None or water is not None or methane is not None:
                    self.cache[country_code] = GridFactors(
                        ...
                        carbon_kg_per_kwh=carbon if carbon is not None else self.WORLD_AVERAGES['carbon'],
                        water_l_per_kwh=water if water is not None else self.WORLD_AVERAGES['water'],
                        methane_g_per_kwh=methane if methane is not None else self.WORLD_AVERAGES['methane'],
    ...
    def get_factors(self, country_code: str = "US") -> GridFactors:
        ...
        logger.warning(f"Country {country_code} not found in grid data, using world averages")
        return GridFactors(
            country_code=country_code,
