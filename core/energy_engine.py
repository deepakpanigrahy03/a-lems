...
class EnergyEngine:
    ...
    # ------------------------------------------------------------------------
    def measure_idle_baseline(self,
                              duration_seconds: int = 10,
                              num_samples: int = 10,
                              pre_wait_seconds: int = 10,
                              force_remeasure: bool = False) -> BaselineMeasurement:
        """
        ...
        print(f"🔍 DEBUG1 - baseline metadata after measure_baseline: {baseline.metadata}")
        ...
        while self._sampling_active:
            try:
                ...
                if sample_counter % 10 == 0:
                    print(f"🔍 DEBUG: counter={sample_counter}, will check interrupt condition")        
                ...
                if sleep_time > 0:
                    ...
                    logger.warning("Sampling loop falling behind; increase interval or reduce load")
                    next_sample = time.time()
    ...
    def start_measurement(self) -> str:
        ...
        interval_ms = int(1000 / self.sampling_rate_hz) if self.sampling_rate_hz > 0 else 100
    ...
    def stop_measurement(self) -> RawEnergyMeasurement:
        ...
        interrupt_samples = []
        if hasattr(self, 'collect_interrupt_samples') and self.collect_interrupt_samples:
        ...
        for key in scheduler_end:
            if key in start_sched:
                if isinstance(scheduler_end[key], (int, float)) and isinstance(start_sched[key], (int, float)):
        ...
        print(f"🔍 DEBUG run queue value: {scheduler_delta.get('runnable', 'NOT FOUND')}")        

        # ====================================================================
        # STEP 4: Calculate actual sampling rate
        # ====================================================================
        actual_sampling_rate = len(samples) / duration_seconds if duration_seconds > 0 else 0
            ...
            try:
                ...
                if hasattr(self.msr, '_cstate_start') and self.msr._cstate_start:
                    ...
                    for state in ['c2', 'c3', 'c6', 'c7']:
                        ...
                        if hasattr(self.msr, 'tsc_frequency_hz') and self.msr.tsc_frequency_hz:
                    ...
                    msr_metrics['cstate_deltas'] = cstate_deltas
                    logger.debug(f"C-state deltas (sec): c2={msr_metrics.get('c2_time_seconds', 0):.3f}")
                
                # Debug prints
                print(f"🟢 ENGINE_ID: {id(msr_metrics)}")
                print(f"🟢 ENGINE_VALUE: c2={msr_metrics.get('c2_time_seconds', 0):.3f}s")
        ...
        if thermal_start and thermal_end:
            ...
            thermal_during_experiment = 1 if end_combined_log > start_combined_log else 0
    ...
    def print_summary(self, measurement: RawEnergyMeasurement):
        ...
        print(f"   Start: {measurement.start_time:.3f}, End: {measurement.end_time:.3f}")
        ...
        if measurement.turbostat and measurement.turbostat.get('dataframe') is not None:
            ...
            if 'frequency_mean' in summary:
                print(f"   Frequency (MHz): mean={summary['frequency_mean']:.0f}, "
        ...
        if measurement.msr_metrics:
            ...
            if ring_bus:
                print(f"   Ring Bus Frequency: {ring_bus.get('current_mhz', 0):.1f} MHz")
...
if __name__ == "__main__":
    ...
    baseline = engine.measure_idle_baseline(duration_seconds=2, num_samples=2, pre_wait_seconds=2)
    ...
    if engine.measurement.metadata.get('baseline_available'):
        print(f"Baseline available: {engine.measurement.metadata['baseline_power_watts']} W")
    ...
    for metric, values in stats['summary'].items():
        print(f"   {metric}: mean={values['mean']:.2f} {values['unit']}, "
