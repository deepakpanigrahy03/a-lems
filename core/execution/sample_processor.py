...
def process_energy_samples(energy_engine) -> tuple:
    ...
    if hasattr(energy_engine, 'last_samples'):
        ...
        logger.debug(f"🔍 DEBUG - Found {len(samples)} samples in energy_engine.last_samples")
        ...
        logger.debug(f"📊 Processed {len(energy_samples)} energy samples, {len(interrupt_samples)} interrupt samples")
...
def process_cpu_samples(raw_energy, canonical_metrics, store_extra=True) -> list:
    ...
    if hasattr(raw_energy, 'turbostat') and raw_energy.turbostat.get('dataframe') is not None:
        ...
        if not df.empty:
            ...
            logger.debug(f"📊 Extracted {len(cpu_samples)} CPU samples with {len(canonical_metrics)} canonical metrics")
            if cpu_samples:
...
def load_canonical_metrics() -> tuple:
    ...
    if override_path.exists():
        try:
            ...
            logger.debug(f"📋 Loaded {len(canonical_metrics)} canonical metrics from override file")
