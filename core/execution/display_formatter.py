...
def display_workflow_comparison(linear_stats, agentic_stats):
    ...
    if linear_stats and agentic_stats:
        ...
        # Energy
        energy_ratio = (agentic_stats['energy'] / linear_stats['energy']) if linear_stats['energy'] else float('nan')
        print(f"   {'Energy (J)':<20} {linear_stats['energy']:>15.6f} {agentic_stats['energy']:>15.6f} "
              f"{energy_ratio:>10.2f}x")
        
        # Carbon
        carbon_ratio = (agentic_stats['carbon'] / linear_stats['carbon']) if linear_stats['carbon'] else float('nan')
        print(f"   {'Carbon (mg)':<20} {linear_stats['carbon']*1000:>15.6f} {agentic_stats['carbon']*1000:>15.6f} "
              f"{carbon_ratio:>10.2f}x")
        
        # Water
        water_ratio = (agentic_stats['water'] / linear_stats['water']) if linear_stats['water'] else float('nan')
        print(f"   {'Water (µl)':<20} {linear_stats['water']*1000:>15.6f} {agentic_stats['water']*1000:>15.6f} "
              f"{water_ratio:>10.2f}x")
        
        # Methane
        methane_ratio = (agentic_stats['methane'] / linear_stats['methane']) if linear_stats['methane'] else float('nan')
