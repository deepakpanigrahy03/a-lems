...
class RepetitionExperiment:
    ...
    def run_comparison(self, linear_exec, agentic_exec, task: str) -> Dict[str, Any]:
        ...
        for i in range(self.n):
            ...
            # Extract metrics
            linear_energies.append(result['linear']['derived_energy']['energy']['workload_energy_j'])
            agentic_energies.append(result['agentic']['derived_energy']['energy']['workload_energy_j'])
        ...
        def calc_stats(data):
            ...
            ci = stats.t.interval(0.95, len(arr)-1, loc=mean, scale=std/np.sqrt(len(arr)))
            return {
                'mean': mean,
                'std': std,
                'ci_lower': ci[0],
