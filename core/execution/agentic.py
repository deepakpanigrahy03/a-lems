...
class AgenticExecutor:
    ...
    def _calculate_complexity_score(self, llm_calls: int, tool_calls: int, total_tokens: int) -> Dict[str, float]:
        """
        ...
        raw_score = (ALPHA * normalized_llm + 
                     BETA * normalized_tools + 
    ...
    def execute(self, task: str, planning_temperature: float = 0.0) -> Dict[str, Any]:
        ...
        for i, step in enumerate(steps):
            ...
            if step.get('tool') in self.supported_tools:
                ...
                if 'usage' in llm_result:
                    ...
                    print(f"🔍 DEBUG - added prompt:{usage.get('prompt_tokens',0)}, completion:{usage.get('completion_tokens',0)}, total:{usage.get('total_tokens',0)}")
                    ...
                    for k, v in llm_result['tokens'].items():
                        tokens[k] += v
                        print(f"🔍 DEBUG - added {k}: {v}, now tokens[{k}] = {tokens[k]}")
                else:
                    logger.debug(f"No token data in llm_result. Keys: {llm_result.keys()}")
        ...
        if hasattr(self, '_effective_kbps_list') and self._effective_kbps_list:
            total_effective_kbps = sum(self._effective_kbps_list) / len(self._effective_kbps_list)
            # Sum all effective_kbps values (you'd need to track them)
            # For now, let's calculate average
            dprint(f"📊 Average throughput: {total_effective_kbps:.1f} kbps across {len(self._effective_kbps_list)} calls")
        ...
        result = {
            ...
            'phase_percentages': {
                'planning_pct': (planning_time_ms / total_time_ms) * 100 if total_time_ms > 0 else 0,
                'execution_pct': (execution_time_ms / total_time_ms) * 100 if total_time_ms > 0 else 0,
                'synthesis_pct': (synthesis_time_ms / total_time_ms) * 100 if total_time_ms > 0 else 0
            },
            'timestamps': {
        ...
        dprint(f"\n📊 Phase breakdown:")
        dprint(f"   Planning:  {planning_time_ms:6.1f}ms ({result['phase_percentages']['planning_pct']:.0f}%)")
        dprint(f"   Execution: {execution_time_ms:6.1f}ms ({result['phase_percentages']['execution_pct']:.0f}%)")
        dprint(f"   Synthesis: {synthesis_time_ms:6.1f}ms ({result['phase_percentages']['synthesis_pct']:.0f}%)")
        dprint(f"   TOTAL:     {total_time_ms:6.1f}ms")
        dprint(f"✅ Agentic complete: {total_time_ms:.0f}ms, {tokens.get('total', 0)} tokens")
        # ====================================================================
        ...
        phase_ratios = {
            'planning_ratio': planning_time_ms / total_time_ms if total_time_ms > 0 else 0,
            'execution_ratio': execution_time_ms / total_time_ms if total_time_ms > 0 else 0,
            'synthesis_ratio': synthesis_time_ms / total_time_ms if total_time_ms > 0 else 0
        }
...
"""
    ...
    def _execute_tool(self, name: str, args: Dict, step_index: int = None) -> Any:
        ...
        if name == "calculator":
            ...
            elif expr == '3*4': result = 12
            elif expr == '10/2': result = 5
            else:
                try:
    ...
    def _emit_event(self, phase: str, event_type: str, start_time: float, 
                   end_time: float, metadata: Dict = None) -> None:
        """
