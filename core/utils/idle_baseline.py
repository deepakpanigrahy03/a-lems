...
def measure_idle_baseline(
    ...
    dprint(f"System state: governor={current_state['governor']}, "
    ...
    if not force_remeasure and cache_file.exists():
        try:
            ...
            if (cached_state.get('governor') == current_state['governor'] and
                cached_state.get('turbo') == current_state['turbo']):
    ...
    dprint(f"📝 Measuring idle baseline: {num_samples} samples of {duration_seconds}s each")
    ...
    # ------------------------------------------------------------------------
    dprint(f"⏳ Waiting {pre_wait_seconds} seconds for system to enter deep idle states...")
    ...
    dprint("✅ Idle baseline complete:", **{k: f"{v:.4f} W" for k, v in avg_power.items()})
    return baseline
...
if __name__ == "__main__":
    ...
    print("\n📝 Second call (should load from cache)...")
    baseline2 = measure_idle_baseline(
        rapl_reader=rapl,
        core_pinner=pinner
    )
