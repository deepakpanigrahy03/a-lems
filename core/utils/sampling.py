...
class HighFrequencySampler:
    ...
    def __init__(self, rapl_reader, sampling_rate_hz: int = 100, 
        ...
        logger.debug(f"Sampler initialized: rate={sampling_rate_hz}Hz, "
    ...
    def start(self) -> None:
        ...
        self.thread = threading.Thread(target=self._sampling_loop, 
                                       name="RAPL-Sampler",
                                       daemon=True)
        self.thread.start()
    ...
    def stop(self) -> List[EnergySample]:
        ...
        logger.info(f"Sampling stopped: {len(samples)} samples in {duration:.2f}s "
        ...
        if actual_rate < self.sampling_rate_hz * 0.8:
            logger.warning(f"Sampling rate too low: {actual_rate:.1f} Hz "
        ...
        while self.running:
            try:
                ...
                if sleep_time > 0:
                    ...
                    logger.warning(f"Sampling loop falling behind (interval "
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            if test_read and any(v != 0 for v in test_read.values()):
                print(f"✅ Using REAL RAPL reader with domains: {list(test_read.keys())}")
    ...
    print(f"📊 Sampler: {sampler.sampling_rate_hz} Hz ({sampler.interval*1000:.2f} ms interval)")
    ...
    if samples:
        ...
        for i, s in enumerate(samples[:3]):
            print(f"   {i+1}: t={s.timestamp:.3f}, PKG={s.package_uj} µJ, "
                  f"CORE={s.core_uj} µJ")
