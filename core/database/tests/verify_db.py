    ...
    try:
        with db:
            ...
            if 'experiments' in tables:
                ...
                name_col = next((c for c in exp_cols if 'name' in c.lower()), exp_cols[1] if len(exp_cols)>1 else exp_cols[0])
                time_col = next((c for c in exp_cols if 'time' in c.lower()), 'timestamp' if 'timestamp' in exp_cols else exp_cols[-1])
            ...
            if 'runs' in tables:
                ...
                id_col = next((c for c in run_cols if 'run_id' in c.lower() or 'id' in c.lower()), run_cols[0])
                exp_id_col = next((c for c in run_cols if 'experiment' in c.lower() and 'id' in c.lower()), None)
                run_num_col = next((c for c in run_cols if 'run_number' in c.lower() or 'num' in c.lower()), None)
                wf_col = next((c for c in run_cols if 'workflow' in c.lower()), None)
                energy_col = next((c for c in run_cols if 'energy' in c.lower() and 'joules' in c.lower()), None)
                success_col = next((c for c in run_cols if 'success' in c.lower()), None)
                
                select_cols = [id_col]
                if exp_id_col: select_cols.append(exp_id_col)
                if run_num_col: select_cols.append(run_num_col)
                if wf_col: select_cols.append(wf_col)
                if energy_col: select_cols.append(energy_col)
                if success_col: select_cols.append(success_col)
            ...
            if 'runs' in tables and 'experiments' in tables:
                exp_id_col = next((c for c in get_column_names(conn, 'experiments') if 'id' in c.lower()), None)
                run_exp_col = next((c for c in get_column_names(conn, 'runs') if 'experiment' in c.lower()), None)
            ...
            exp_count = conn.execute("SELECT COUNT(*) FROM experiments").fetchone()[0] if 'experiments' in tables else 0
