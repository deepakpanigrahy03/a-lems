...
from ..base import DatabaseInterface, DatabaseError
...
class RunsRepository:
    ...
    def _extract_from_ml_features(self, ml: Dict[str, Any]) -> Dict[str, Any]:
        ...
        thermal_delta = (max_temp - start_temp) if (start_temp is not None and max_temp is not None) else None
    ...
    def insert_run(self, 
        ...
        params = (
            ...
            fields['context_switches_voluntary'], fields['context_switches_involuntary'],
            fields['total_context_switches'],
            fields['thread_migrations'], fields['run_queue_length'],
            fields['kernel_time_ms'], fields['user_time_ms'],
            fields['frequency_mhz'], fields['ring_bus_freq_mhz'],
            fields['package_temp_celsius'], fields['baseline_temp_celsius'],
            fields['start_temp_c'], fields['max_temp_c'], fields['min_temp_c'],  fields['thermal_delta_c'],
            fields['thermal_during_experiment'], fields['thermal_now_active'],
            ...
            fields['dns_latency_ms'], fields['api_latency_ms'], fields['compute_time_ms'],
            fields['governor'], fields['turbo_enabled'], fields['is_cold_start'],
            fields['background_cpu_percent'], fields['process_count'],
            fields['planning_time_ms'], fields['execution_time_ms'], fields['synthesis_time_ms'],
            fields['phase_planning_ratio'], fields['phase_execution_ratio'],
            fields['phase_synthesis_ratio'],
            fields['llm_calls'], fields['tool_calls'], fields['tools_used'],
            fields['steps'], fields['avg_step_time_ms'],
            fields['complexity_level'], fields['complexity_score'],
            carbon_g, water_ml, methane_mg,
        ...
        try:
            ...
            try:
                exp_check = self.db.execute("SELECT COUNT(*) as count FROM experiments WHERE exp_id = ?", (exp_id,))
                print(f"   experiments count for exp_id {exp_id}: {exp_check[0]['count']}")
            except Exception as ex:
                print(f"   ❌ Failed to check experiments: {ex}")
            
            try:
                hw_check = self.db.execute("SELECT COUNT(*) as count FROM hardware_config WHERE hw_id = ?", (hw_id,))
                print(f"   hardware_config count for hw_id {hw_id}: {hw_check[0]['count']}")
            except Exception as ex:
                print(f"   ❌ Failed to check hardware: {ex}")
            
            if baseline_id:
                try:
                    bl_check = self.db.execute("SELECT COUNT(*) as count FROM idle_baselines WHERE baseline_id = ?", (baseline_id,))
                    print(f"   idle_baselines count for baseline_id {baseline_id}: {bl_check[0]['count']}")
                except Exception as ex:
