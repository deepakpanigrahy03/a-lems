...
class SQLiteAdapter(DatabaseInterface):
    ...
    def insert_baseline(self, baseline_data: Dict[str, Any]) -> str:
        ...
        baseline_id = baseline_data.get('baseline_id') or f"baseline_{int(datetime.now().timestamp())}"
        ...
        print(f"🔍 DEBUG sqladapter- governor from metadata: {metadata.get('governor')}")
    ...
    def get_runs_by_experiment(self, exp_id: int) -> List[Dict[str, Any]]:
        ...
        return self.execute(
            "SELECT * FROM runs WHERE exp_id = ? ORDER BY run_number",
            (exp_id,)
        )
    ...
    def get_ml_data(self, workflow: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if workflow:
            return self.execute(
                "SELECT * FROM ml_features WHERE workflow_type = ?",
                (workflow,)
            )
