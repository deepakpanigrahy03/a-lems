...
class ConfigLoader:
    ...
    def __init__(self, config_dir: Optional[str] = None):
        ...
        print(f"✅ Loaded models config: {list(self._models_config.keys()) if self._models_config else 'None'}")
        
        # Load hardware config
        self._hardware_config = self._load_json("hw_config.json")
        print(f"✅ Loaded hardware config: {list(self._hardware_config.keys()) if self._hardware_config else 'None'}")
        ...
        if self._grid_intensity:
            country_count = len([k for k in self._grid_intensity.keys() if k != "metadata"])
            ...
            if 'IN' in self._grid_intensity:
                ...
                print(f"   Carbon: {self._grid_intensity['IN'].get('carbon_intensity')}")
            else:
