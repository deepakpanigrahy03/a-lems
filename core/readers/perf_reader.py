...
class PerfReader:
    ...
    def __init__(self, config: Dict):
        ...
        if self.perf_available:
            logger.info("PerfReader initialized with %d events: %s", 
                       len(self.events), self.events[:5])
    ...
    def _check_perf_availability(self) -> bool:
        ...
        try:
            cmd = [
                ...
                '--field-separator=,', '--no-big-num',
                '--timeout', '10', 'sleep', '0.01'
            ]
            result = subprocess.run(
                cmd,
                capture_output=True,
    ...
    def _drain_perf_output(self):
                ...
                if reads:
                    ...
                    if line:
                        ...
                        if lines_read % 50 == 0:
                            ...
                            logger.debug(f"Perf drainer collected {lines_read} lines, buffer: {buffer_size}KB")
                    else:
        ...
        buffer_size = len(''.join(self._perf_buffer)) // 1024 if self._perf_buffer else 0
        logger.info(f"Perf output drainer stopped, collected {lines_read} lines, buffer: {buffer_size}KB")
    ...
    def read_counters(self, duration_ms: int = 100) -> PerformanceCounters:
        ...
        try:
            ...
            logger.debug("perf counters: %d instructions, %d cycles, %.2f IPC",
    ...
    def read_interval_counters(self, interval_ms: int = 100, 
                               samples: int = 10) -> List[PerformanceCounters]:
        """
    ...
    def _parse_perf_output(self, output: str, counters: PerformanceCounters) -> None:
        ...
        for line in output.split('\n'):
            line = line.strip()
            if not line or any(x in line.lower() for x in ['performance', 'seconds', 'cpus utilized']):
                continue
            ...
            try:
                ...
                continue    
    def _map_event_to_counter(self, event_name: str, value: int, 
                             counters: PerformanceCounters) -> None:
        """
...
if __name__ == "__main__":
    ...
    for i, c in enumerate(samples):
        print(f"   Sample {i+1}: {c.instructions_retired:,} instructions, {c.instructions_per_cycle():.2f} IPC")
