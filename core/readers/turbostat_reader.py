...
class TurbostatReader:
    ...
    def __init__(self, config: Dict):
        ...
        if self.cpu_topology.get('has_hybrid'):
            logger.info(f"Hybrid CPU detected: {len(self.cpu_topology.get('p_cores', []))} P-cores, "
    ...
    def _find_turbostat(self) -> Optional[str]:
        ...
        try:
            result = subprocess.run(['which', 'turbostat'], capture_output=True, text=True)
                        ...
                        if len(parts) > 1:
                            ...
                            if 'P-core' in core_info:
                                ...
                                for r in p_ranges:
                                    ...
                                    if '-' in r:
                                        ...
                                        topology['p_cores'].extend(list(range(start, end+1)))
                                    else:
                            ...
                            if 'E-core' in core_info:
                                ...
                                for r in e_ranges:
                                    if '-' in r:
                                        start, end = map(int, r.split('-'))
                                        topology['e_cores'].extend(list(range(start, end+1)))
                                    else:
    ...
    def _drain_output(self):
        ...
        while not self._stop_reader:
            try:
                ...
                reads, _, _ = select.select([self.turbostat_process.stdout], [], [], 0.1)
                ...
                if reads:
                    ...
                    if line:
                        ...
                        if lines_read <= 5:
                            logger.debug(f"✅ Turbostat line {lines_read}: {line.strip()[:100]}")
                        elif lines_read == 6:
                        ...
                        if lines_read % 50 == 0:
                            ...
                            logger.debug(f"Turbostat drainer collected {lines_read} lines, buffer: {buffer_size}KB")
                    else:
        ...
        buffer_size = len(''.join(self._output_buffer)) // 1024 if self._output_buffer else 0
        logger.info(f"Turbostat output drainer stopped, collected {lines_read} lines, buffer: {buffer_size}KB")
    ...
    def start_monitoring(self, interval_ms: int = 100):
        ...
        if not self.available:
            logger.warning("turbostat not available, cannot start continuous monitoring")
            return
        ...
        cmd = [
            ...
            '--quiet',
            '--Summary', 
            '--show', ','.join(self.column_map.values()),
        ...
        logger.info(f"turbostat continuous monitoring started (interval={interval_ms}ms)")
    ...
    def read_metrics(self, duration_ms: int = 100) -> PowerState:
        ...
        cmd = [
            ...
            '--quiet',
            '--Summary', 
            '--show', ','.join(columns_to_read),
            '--interval', str(interval),
            'sleep', str(interval + 0.1)
        ]
    ...
    def _parse_turbostat_output(self, output: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            df = pd.read_csv(io.StringIO(data_lines), sep='\t', 
...
if __name__ == "__main__":
    ...
    if reader.cpu_topology.get('has_hybrid'):
        print(f"🔧 Hybrid CPU detected: {len(reader.cpu_topology.get('p_cores', []))} P-cores, "
