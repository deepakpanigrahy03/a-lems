...
class RAPLReader:
    ...
    def __init__(self, config: Dict):
        ...
        logger.info("Initializing RAPLReader with %d configured domains", 
                   len(self.rapl_paths))
        ...
        for domain_name, path in self.rapl_paths.items():
            if self._validate_path(path):
                ...
                logger.debug("RAPL domain available: %s -> %s (%s)", 
                           domain_name, std_name, path)
            else:
                logger.warning("RAPL domain not accessible: %s at %s", 
                             domain_name, path)
        
        # Log summary
        if self.available_paths:
            logger.info("RAPLReader initialized with %d accessible domains: %s", 
    ...
    def read_with_retry(self, max_retries: int = 3) -> Dict[str, int]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    logger.error("RAPL read failed after %d attempts: %s", 
                               max_retries, e)
                    return {}
    ...
    def get_energy_delta(self, start_readings: Dict[str, int], 
                        end_readings: Dict[str, int]) -> Dict[str, int]:
        """
        ...
        for domain, start_val in start_readings.items():
            if domain in end_readings:
                ...
                if end_val >= start_val:
                    ...
                    logger.debug("RAPL counter wrapped for %s: %d -> %d, delta=%d",
                               domain, start_val, end_val, delta)
