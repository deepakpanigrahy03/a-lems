...
class MSRReader:
    ...
    def __init__(self, config: Dict[str, Any], use_baseline: bool = True, require_baseline: bool = False):
        """
        ...
        if self.ring_bus_sysfs_paths:
            logger.debug(f"Loaded ring bus sysfs paths: {list(self.ring_bus_sysfs_paths.keys())}")
        ...
        if self.tsc_frequency_hz:
            logger.info(f"TSC frequency: {self.tsc_frequency_hz/1e6:.0f} MHz (from config)")
        else:
            logger.warning("TSC frequency not in config - C-state conversions will be approximate")
        ...
        self.log_interval = settings.get('logging', {}).get('msr_progress_interval', 100)
    ...
    def _check_helper(self):
        ...
        if self.HELPER_PATH.exists() and os.access(self.HELPER_PATH, os.X_OK):
            ...
            self.helper_available = False
            logger.warning(f"C helper not found at {self.HELPER_PATH}. Will use direct access (may need sudo).")
    ...
    # ------------------------------------------------------------------------
    def read_cstate_counters_for_wakeup(self, cpu: int = 0, pin: bool = True) -> Dict[str, int]:
        """
    ...
    def calculate_wakeup_delta(self, start_counters: Dict[str, int], 
        ...
        logger.debug(f"CPU wakeup delta: {wakeups} (C6: {delta_c6 if 'c6' in locals() else 0}, "
    ...
    # ------------------------------------------------------------------------
    def get_ring_limits(self, cpu: int = 0, pin: bool = True) -> Optional[Dict[str, float]]:
        """
        ...
        return {
            'min_mhz': min_ratio * base_clock,
            'max_mhz': max_ratio * base_clock
        }
        ...
        if current_path and os.path.exists(current_path):
            try:
                with open(current_path, 'r') as f:
                    ...
                    dprint(f"✅ RING BUS DEBUG: Read {freq_khz} kHz from {current_path}")
                    logger.debug(f"Current ring bus freq: {freq_khz} kHz from {current_path}")
        ...
        if init_max_path and os.path.exists(init_max_path):
            try:
                with open(init_max_path, 'r') as f:
                    freq_khz = int(f.read().strip())
                    dprint(f"✅ RING BUS DEBUG: Read {freq_khz} kHz from {init_max_path}")
                    logger.debug(f"Initial max ring bus freq: {freq_khz} kHz from {init_max_path}")
                    return freq_khz / 1000.0
            except (IOError, OSError, ValueError) as e:
    ...
    def measure_ring_bus_frequency(self) -> Optional[float]:
        ...
        if limits:
            ...
            dprint(f"⚠️ RING BUS DEBUG: MSR average = {avg_freq:.1f} MHz (min={limits['min_mhz']:.0f}, max={limits['max_mhz']:.0f})")
            logger.debug(f"Ring bus frequency (MSR average): {avg_freq:.1f} MHz "
    ...
    def read_cstate_counters_all_cpus(self, pin: bool = True) -> Dict[int, Dict[str, Any]]:
    ...
    def average_cstate_counters(self, counters_per_cpu: Dict[int, Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    # ------------------------------------------------------------------------
    def read_thermal_throttle_status(self, cpu: int = 0, pin: bool = True) -> Optional[Dict[str, int]]:
        """
        ...
        """
        val = self.read_msr(
            self.MSR_ADDRESSES['MSR_IA32_PACKAGE_THERM_STATUS'],
            cpu,
            pin=pin
        )
    ...
    def read_core_thermal_status(self, cpu: int = 0, pin: bool = True) -> Optional[Dict[str, int]]:
        """
        ...
        """
        val = self.read_msr(
            self.MSR_ADDRESSES['MSR_IA32_THERM_STATUS'],
            cpu,
            pin=pin
        )
    ...
    def snapshot_thermal_state(self, cpu: int = 0, pin: bool = True) -> Optional[Dict[str, Any]]:
        """
        ...
        if package_status is None and core_status is None:
            logger.warning("Both package and core thermal MSRs failed - cannot determine thermal state")
            return None
        
        # Use defaults (0,0) for any failed reads to prevent crashes
        safe_package = package_status if package_status is not None else {"thermal_now": 0, "thermal_log": 0}
        safe_core = core_status if core_status is not None else {"thermal_now": 0, "thermal_log": 0}
        from datetime import datetime
        return {
            "package": safe_package,
            "core": safe_core,
            "timestamp": time.time(),
            "human_time": datetime.fromtimestamp(time.time()).strftime("%Y-%m-%d %H:%M:%S")
        }
    # ------------------------------------------------------------------------
    # APERF/MPERF frequency calculation - REAL CPU frequency
    # ------------------------------------------------------------------------
    def calculate_actual_frequency(self, aperf1: int, aperf2: int, mperf1: int, mperf2: int) -> Optional[float]:
        """
    ...
    def get_all_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate averages
            metrics['dynamic']['cstate_averages'] = self.average_cstate_counters(cstate_all)

            # ====================================================================
            # Calculate per-run C-state deltas if start snapshot exists
            # ====================================================================
            print(f"🔵 MSR_READER - _cstate_start exists: {hasattr(self, '_cstate_start')}")
            ...
            if self.start_cstate_counters and self.end_cstate_counters:
                ...
                dprint(f"   Start counters: {self.start_cstate_counters.get('counters', {})}")
                dprint(f"   End counters: {self.end_cstate_counters.get('counters', {})}")
            ...
            dprint(f"   metrics['dynamic']['thermal'] = {metrics['dynamic']['thermal']}")
    ...
    # ------------------------------------------------------------------------
    def measure_hardware_wakeup_latency(self, cpu: int = 0, iterations: Optional[int] = None) -> Optional[float]:
        """Measure wake-up latency using C-state counters (baseline only)."""
        if not self.helper_available and not self.rdmsr_available:
            logger.warning("MSR not available, cannot measure hardware wake-up latency.")
            return None
        ...
        for i in range(iterations):
            c7_before = self.read_msr(self.MSR_ADDRESSES['MSR_PKG_C7_RESIDENCY'], cpu, pin=False)
            ...
            c7_after = self.read_msr(self.MSR_ADDRESSES['MSR_PKG_C7_RESIDENCY'], cpu, pin=False)
    ...
    def __str__(self) -> str:
        status = "available" if (self.helper_available or self.rdmsr_available) else "unavailable"
        method = "C helper" if self.helper_available else "direct" if self.rdmsr_available else "none"
        pinned = f", pinned to CPU {self._pinned_cpu}" if self._pinned_cpu is not None else ""
        baseline_info = f", baseline loaded: {'yes' if self.baseline else 'no'}"
        tsc_info = f", TSC: {self.tsc_frequency_hz/1e6:.0f}MHz" if self.tsc_frequency_hz else ", TSC: unknown"
