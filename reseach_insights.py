...
# Page config
st.set_page_config(
    page_title="A-LEMS Research Insights",
    page_icon="📊",
    layout="wide"
)
...
# Create tabs
tab1, tab2, tab3 = st.tabs(["🎯 Guided Insights", "🔧 Custom Query Lab", "📖 Schema Reference"])
...
with tab1:
    ...
    selected_category = st.selectbox(
        "Select Category",
        categories,
        help="Choose a research category"
    )
    
    # Get questions for selected category
    category_data = next(c for c in questions_config["categories"] 
    ...
    question_data = next(q for q in category_data["questions"] 
    ...
    with st.expander("🔍 Filters", expanded=False):
        ...
        with col1:
            providers = pd.read_sql("SELECT DISTINCT provider FROM experiments WHERE provider IS NOT NULL", conn)['provider'].tolist()
            selected_providers = st.multiselect("Providers", providers, default=providers)
        
        with col2:
            workflows = ['linear', 'agentic']
            selected_workflows = st.multiselect("Workflows", workflows, default=workflows)
        ...
        with st.spinner("Running analysis..."):
            ...
            with col1:
                ...
                if "insight_template" in question_data:
                    ...
                    if not df.empty:
                        insight = question_data["insight_template"].format(**df.iloc[0].to_dict())
                        st.info(insight)
                    else:
                    ...
                    if isinstance(question_data["y_axis"], list):
                        ...
                        if "error_bars" in question_data:
                            fig.update_traces(error_y=dict(
                                type='data',
                                array=df[question_data["error_bars"]]
                            ))
            ...
            try:
                with st.spinner("Executing query..."):
                    ...
                    if not df.empty:
                        ...
                        with col3:
                            st.button("📋 Copy SQL", on_click=lambda: st.write("SQL copied!"))
...
with tab3:
    ...
    for table in tables:
        with st.expander(f"📊 {table}"):
            ...
            st.code(f"SELECT * FROM {table} WHERE run_id IN (SELECT run_id FROM runs LIMIT 5);", language="sql")
