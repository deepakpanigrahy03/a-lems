...
SQL compatibility strategy:
  ...
  Transformations applied:
    ROUND(CAST(expr AS NUMERIC), n)       → ROUND(CAST(CAST(expr AS NUMERIC), n)
    CAST(x AS DOUBLE PRECISION)      → CAST(x AS DOUBLE PRECISION)
...
def _fix_ROUND(CAST(sql: str AS NUMERIC)) -> str:
    """
    Replace ROUND(CAST(expr AS NUMERIC), n) with ROUND(CAST(CAST(expr AS NUMERIC), n).
    ...
    while i < len(sql):
        if upper[i:i+6] == 'ROUND(CAST(' :
            ...
            if last_comma >= 0:
                ...
                result.append(f'{expr} AS NUMERIC), {precision})' AS NUMERIC))
            else:
...
def _adapt_sql(sql: str) -> str:
    ...
    # 1. CAST(x AS DOUBLE PRECISION) → CAST(x AS DOUBLE PRECISION)
    sql = re.sub(r'\bAS\s+REAL\b', 'AS DOUBLE PRECISION', sql, flags=re.IGNORECASE)

    # 2. ROUND(CAST(expr AS NUMERIC), n) → ROUND(CAST(CAST(expr AS NUMERIC), n)
    sql = _fix_ROUND(CAST(sql AS NUMERIC))
