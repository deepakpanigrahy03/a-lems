...
def render(ctx: dict) -> None:
    ...
    # ── Header ────────────────────────────────────────────────────────────────
    health_clr = (
        "#22c55e" if clean_pct >= 90 else "#f59e0b" if clean_pct >= 70 else "#ef4444"
    )
    st.markdown(
    ...
    if flagged_df.empty:
        ...
        def _flags(row):
            f = []
            if row.get("experiment_valid") == 0:
                f.append("INVALID")
            if row.get("thermal_throttle_flag") == 1:
                f.append("THROTTLE")
            if pd.isna(row.get("baseline_id")):
                f.append("NO_BASELINE")
            if row.get("thermal_during_experiment") == 1:
                f.append("THERMAL_EVENT")
            if (row.get("background_cpu_percent") or 0) > 20:
                f.append("NOISY")
            return " · ".join(f)
