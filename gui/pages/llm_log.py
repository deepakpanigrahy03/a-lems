...
def render(ctx: dict) -> None:
        ...
        + "".join(
            [
                ...
                for v, l, c in [
                    ...
                    (
                        f"{stats.get('avg_latency',0):.0f}ms",
                        "Avg API latency",
                        "#a78bfa",
                    ),
                    (
                        f"{int(stats.get('total_tokens',0)/1e6):.1f}M",
                        "Total tokens",
                        "#22c55e",
                    ),
                ]
    ...
    if sel_run != "All":
        where_parts.append(f"run_id = {sel_run}")
    ...
    with col2:
        ...
        fig2.update_layout(
            **PL, height=240, yaxis_title="Total tokens", showlegend=False
        )
    ...
    if sel_run != "All":
        st.markdown(
            ...
            f"Step timeline — Run {sel_run}</div>",
            unsafe_allow_html=True,
        )
        steps = q(f"""
        ...
        if not steps.empty:
            ...
            st.plotly_chart(fig3, use_container_width=True, key=f"llm_steps_{sel_run}")

    # ── Interaction log table ─────────────────────────────────────────────────
    st.markdown(
        f"<div style='font-size:11px;font-weight:600;color:{ACCENT};"
        f"text-transform:uppercase;letter-spacing:.1em;margin:16px 0 8px;'>"
        f"Interaction log — latest 200</div>",
        unsafe_allow_html=True,
    )
