...
def render(ctx: dict) -> None:
    ...
    throttle_count = (
        q1("SELECT COUNT(*) AS n FROM thermal_samples WHERE throttle_event=1").get(
            "n", 0
        )
        or 0
    )
    ...
    # ── Header ────────────────────────────────────────────────────────────────
    throttle_pct = (
        round(throttle_count / total_samples * 100, 1) if total_samples else 0
    )
    health_clr = (
        "#22c55e"
        if throttle_pct < 10
        else "#f59e0b" if throttle_pct < 50 else "#ef4444"
    )
    ...
    sel_run = st.selectbox(
        "Select run", run_ids, key="th_run_sel", format_func=lambda x: f"Run {x}"
    )
