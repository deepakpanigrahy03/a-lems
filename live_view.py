...
def apply_theme(fig, **kwargs):
    fig.update_layout(
        ...
        font=dict(family="IBM Plex Mono, monospace", size=10, color=COLORS['text_muted']),
        margin=dict(l=40, r=20, t=30, b=30),
        legend=dict(bgcolor="rgba(0,0,0,0)", font=dict(size=9)),
        colorway=[COLORS['linear'], COLORS['agentic'], COLORS['execution'],
                  COLORS['planning'], COLORS['synthesis'], COLORS['c3']],
        xaxis=dict(gridcolor=COLORS['border'], linecolor=COLORS['border'], tickfont=dict(size=9)),
        yaxis=dict(gridcolor=COLORS['border'], linecolor=COLORS['border'], tickfont=dict(size=9)),
        **kwargs
    )
    return fig
...
if page_id == "overview":
    ...
    kpi_cols[1].metric("Tax Multiple", f"{tax_mult:.1f}×",
                       delta=f"{(tax_mult-1)*100:.0f}% overhead", delta_color="inverse")
    kpi_cols[2].metric("Avg Planning", f"{plan_ms:.0f}ms",
                       delta=f"{plan_pct:.0f}%", delta_color="inverse")
    ...
    if not runs.empty:
        ...
        with col1:
            ...
            fig = px.scatter(df, x="duration_s", y="energy_j",
                             color="workflow_type", color_discrete_map={"linear": COLORS['linear'], "agentic": COLORS['agentic']},
        ...
        with col2:
            ...
            fig2 = px.scatter(df2, x="cache_miss_pct", y="ipc",
                              color="workflow_type", color_discrete_map={"linear": COLORS['linear'], "agentic": COLORS['agentic']},
...
# =============================================================================
    ...
    if not runs.empty:
        ...
        with col1:
            ...
            sr["idx"] = sr.index
            fig = px.bar(sr, x="idx", y="energy_j", log_y=True,
                         color="workflow_type", color_discrete_map={"linear": COLORS['linear'], "agentic": COLORS['agentic']},
                         hover_data=["run_id", "provider", "task_name"],
        ...
        with col2:
            ...
            if "carbon_g" in runs.columns:
                ...
                cd["group"] = cd["provider"].fillna("?") + "·" + cd["country_code"].fillna("?")
                cd["carbon_mg"] = cd["carbon_g"] * 1000
                ca = cd.groupby("group")["carbon_mg"].mean().reset_index()
                fig3 = px.bar(ca, x="group", y="carbon_mg", log_y=True, color="group",
                              labels={"carbon_mg": "avg mg CO₂e", "group": ""})
                st.plotly_chart(apply_theme(fig3, showlegend=False), use_container_width=True)
                st.caption("IN grid (0.82 kg/kWh) ≈ 2× US factor — same energy, double carbon.")

        st.divider()
        if "api_latency_ms" in runs.columns:
            cloud = runs[(runs.provider != "local") & runs.api_latency_ms.notna() & runs.energy_j.notna()].copy()
            cloud["api_latency_s"] = cloud["api_latency_ms"] / 1000
            if not cloud.empty:
                st.markdown("**Energy vs API latency**")
                fig4 = px.scatter(cloud, x="api_latency_s", y="energy_j", log_y=True,
                                  color="country_code",
...
# =============================================================================
    ...
    if not cstate_agg.empty:
        ...
        cstate_colors = {"C0": COLORS['c0'], "C1": COLORS['c1'], "C2": COLORS['c2'],
                         "C3": COLORS['c3'], "C6": COLORS['c6'], "C7": COLORS['c7']}
        for _, row in cstate_agg.iterrows():
            ...
            fig = px.bar(data, x="Residency%", y="State", orientation="h",
    ...
    if not runs.empty:
        ...
        with col1:
            ...
            fig = px.histogram(ri, x="ipc", color="workflow_type",
                               color_discrete_map={"linear": COLORS['linear'], "agentic": COLORS['agentic']},
        ...
        with col2:
            ...
            if "cache_miss_rate" in runs.columns and "energy_j" in runs.columns:
                ...
                fig2 = px.scatter(rm, x="cache_miss_pct", y="energy_j", log_y=True,
                                  color="workflow_type",
                                  color_discrete_map={"linear": COLORS['linear'], "agentic": COLORS['agentic']},
                                  hover_data=["run_id", "provider"],
...
# =============================================================================
    ...
    if not runs.empty and "thread_migrations" in runs.columns:
        ...
        c4.metric("Max IRQ/s", f"{sc.interrupt_rate.max():,.0f}" if "interrupt_rate" in sc.columns else "—")
        ...
        with col1:
            ...
            fig = px.scatter(sm, x="duration_s", y="thread_migrations",
                             color="workflow_type",
                             color_discrete_map={"linear": COLORS['linear'], "agentic": COLORS['agentic']},
                             hover_data=["run_id", "provider"],
        ...
        with col2:
            ...
            if "cache_miss_rate" in sc.columns:
                ...
                fig2 = px.scatter(sm2, x="thread_migrations", y="cache_miss_pct",
                                  color="workflow_type",
                                  color_discrete_map={"linear": COLORS['linear'], "agentic": COLORS['agentic']},
                                  hover_data=["run_id"],
...
# =============================================================================
    ...
    if err:
        st.warning("View `orchestration_analysis` not found. Run the following SQL to create it:")
        st.code("""
...
WHERE r.baseline_id IS NOT NULL;
    ...
    if not domains.empty:
        ...
        with col1:
            ...
            tp = domains.head(30)
            fig = go.Figure()
            for col, color, name in [("core_energy_j", COLORS['power_core'], "Core"),
        ...
        with col2:
            ...
            fig2 = go.Figure()
            for col, color, name in [("workload_energy_j", COLORS['linear'], "Workload"),
        ...
        cols = [c for c in ["run_id", "workflow_type", "task_name", "pkg_energy_j", "core_energy_j",
        ...
        st.info("No domain data available. Create the view and ensure runs have baselines.")
...
# =============================================================================
    ...
    if not tax.empty:
        ...
        st.divider()
        cols = [c for c in ["comparison_id", "task_name", "provider", "country_code",
        ...
        with col2:
            if "llm_calls" in tax.columns:
                ...
                fig2 = px.scatter(tx, x="llm_calls", y="tax_percent",
...
# =============================================================================
    ...
    task_list = query("SELECT DISTINCT task_name FROM experiments WHERE task_name IS NOT NULL ORDER BY task_name")
    ...
    tab_batch, tab_single = st.tabs(["⚡ Batch — run_experiment", "🔬 Single — test_harness"])

    with tab_batch:
        col_cfg, col_out = st.columns([1, 2])
        with col_cfg:
            tasks_in = st.text_input("Task IDs (comma‑separated or 'all')", value="simple,capital")
            providers = st.multiselect("Providers", ["cloud", "local"], default=["cloud"], key="batch_prov")
            reps = st.number_input("Repetitions", 1, 100, 3, key="batch_reps")
            country = st.selectbox("Grid region", ["US","DE","FR","NO","IN","AU","GB","CN","BR"], key="batch_country")
            cooldown = st.number_input("Cool‑down (s)", 0, 120, 5, step=5, key="batch_cd")
            ...
            cmd = ["python", "-m", "core.execution.tests.run_experiment",
                   "--tasks", tasks_in.strip(),
                   "--providers", prov_arg,
                   "--repetitions", str(int(reps)),
                   "--country", country,
                   "--cool-down", str(int(cooldown))]
            if save_db: cmd.append("--save-db")
            if optimizer: cmd.append("--optimizer")
            if no_warmup: cmd.append("--no-warmup")
            if out_file.strip(): cmd += ["--output", out_file.strip()]

            st.divider()
            st.markdown("**Command**")
            st.code(" \\\n  ".join(cmd))
            run_btn = st.button("▶ Run batch", type="primary", use_container_width=True, key="batch_run")
            list_btn = st.button("📋 List tasks", use_container_width=True, key="batch_list")

        with col_out:
            if list_btn:
                res = subprocess.run(["python", "-m", "core.execution.tests.run_experiment", "--list-tasks"],
                ...
                if not runs.empty:
                    cols = ["run_id","workflow_type","task_name","provider","country_code","energy_j","ipc"]
                    cols = [c for c in cols if c in runs.columns]
                    st.dataframe(runs.head(20)[cols], use_container_width=True, hide_index=True)
    ...
    with tab_single:
        ...
        with col_cfg:
            ...
            country = st.selectbox("Grid region", ["US","DE","FR","NO","IN","AU","GB","CN","BR"], key="single_country")
            cooldown = st.number_input("Cool‑down (s)", 0, 120, 5, step=5, key="single_cd")
            ...
            st.markdown("**Command**")
            st.code(" \\\n  ".join(cmd))
            run_btn = st.button("▶ Run single", type="primary", use_container_width=True, key="single_run")
            list_btn = st.button("📋 List tasks", use_container_width=True, key="single_list")

        with col_out:
            if list_btn:
                res = subprocess.run(["python", "-m", "core.execution.tests.test_harness", "--list-tasks"],
                                     capture_output=True, text=True, cwd=str(PROJECT_ROOT), timeout=30)
    ...
    if runs.empty:
        ...
        if not es.empty and len(es) > 2:
            ...
            with col1:
                ...
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=es.elapsed_ms, y=es.pkg_w_inst, name="PKG",
                                         line=dict(color=COLORS['power_pkg'], width=1.5)))
                fig.add_trace(go.Scatter(x=es.elapsed_ms, y=es.core_w_inst, name="Core",
                                         line=dict(color=COLORS['power_core'], width=1)))
                st.plotly_chart(apply_theme(fig, xaxis_title="ms", yaxis_title="Watts"), use_container_width=True)
            with col2:
                st.markdown("**Cumulative energy (J)**")
                fig2 = go.Figure()
                fig2.add_trace(go.Scatter(x=es.elapsed_ms, y=es.pkg_j, name="PKG", line=dict(color=COLORS['power_pkg'])))
                fig2.add_trace(go.Scatter(x=es.elapsed_ms, y=es.core_j, name="Core", line=dict(color=COLORS['power_core'])))
                st.plotly_chart(apply_theme(fig2, xaxis_title="ms", yaxis_title="Joules"), use_container_width=True)
        ...
        if not cs.empty:
            ...
            with col1:
                ...
                fig3.add_trace(go.Scatter(x=cs.elapsed_ms, y=cs.ipc, name="IPC",
                                          line=dict(color=COLORS['linear'], width=1.5)), secondary_y=False)
                fig3.add_trace(go.Scatter(x=cs.elapsed_ms, y=cs.cpu_util_percent, name="Util%",
                                          line=dict(color=COLORS['planning'], width=1)), secondary_y=True)
                fig3.update_layout(**apply_theme(fig3).layout)
                fig3.update_yaxes(title_text="IPC", secondary_y=False, gridcolor=COLORS['border'])
                fig3.update_yaxes(title_text="Util%", secondary_y=True, gridcolor=COLORS['border'])
                st.plotly_chart(fig3, use_container_width=True)
            with col2:
                st.markdown("**C‑States**")
                fig4 = go.Figure()
                for col, color, name in [("c7", COLORS['c7'], "C7"), ("c6", COLORS['c6'], "C6"),
                                         ("c3", COLORS['c3'], "C3"), ("c2", COLORS['c2'], "C2"),
                                         ("c1", COLORS['c1'], "C1")]:
                    if col in cs.columns:
                        fig4.add_trace(go.Scatter(x=cs.elapsed_ms, y=cs[col], name=name,
                                                  line=dict(color=color, width=1),
                                                  stackgroup="cstate", fill="tonexty"))
                st.plotly_chart(apply_theme(fig4, xaxis_title="ms", yaxis_title="Residency %"), use_container_width=True)

        # Interrupts
        if not irq.empty:
            st.markdown("**IRQ rate**")
            fig5 = go.Figure()
            fig5.add_trace(go.Scatter(x=irq.elapsed_ms, y=irq.interrupts_per_sec, name="IRQ/s",
                                      line=dict(color=COLORS['agentic'], width=1.5)))
            st.plotly_chart(apply_theme(fig5, xaxis_title="ms", yaxis_title="IRQ/s"), use_container_width=True)

        # Events
        if not ev.empty:
            st.markdown("**Orchestration Events**")
            phase_colors = {"planning": COLORS['planning'], "execution": COLORS['execution'], "synthesis": COLORS['synthesis']}
            fig6 = go.Figure()
            ...
            fig6.update_layout(**apply_theme(fig6).layout, showlegend=False, height=max(200, len(ev)*30))
...
# =============================================================================
    ...
    """)
    if not exps.empty:
        cols = ["exp_id","name","task_name","provider","country_code","status","run_count","avg_linear_j","avg_agentic_j"]
        cols = [c for c in cols if c in exps.columns]
        st.dataframe(exps[cols], use_container_width=True, hide_index=True)
        st.divider()
        selected = st.selectbox("Inspect experiment", exps.exp_id.tolist(),
                                format_func=lambda x: f"Exp {x} — {exps[exps.exp_id==x]['name'].values[0]}")
        exp_runs = query(f"SELECT * FROM runs WHERE exp_id={selected} ORDER BY run_number")
        exp_tax = query(f"""
        ...
        if not exp_runs.empty:
            lin_avg = exp_runs[exp_runs.workflow_type=="linear"].total_energy_uj.mean()/1e6
            age_avg = exp_runs[exp_runs.workflow_type=="agentic"].total_energy_uj.mean()/1e6
            c1,c2,c3,c4 = st.columns(4)
            ...
            run_cols = ["run_id","workflow_type","run_number","total_energy_uj","ipc","cache_miss_rate","thread_migrations","carbon_g"]
        ...
        if not exp_tax.empty:
            ...
            tax_cols = ["comparison_id","linear_dynamic_j","agentic_dynamic_j","tax_j","tax_percent"]
...
# =============================================================================
    ...
    default_query = templates[choice] if choice != "Custom" else "SELECT * FROM runs LIMIT 50"
    ...
    for f in sorted(CONFIG_DIR.glob("*")):
        if f.is_file() and f.suffix in [".yaml", ".yml", ".json"]:
            with st.expander(f.name):
                try:
                    content = f.read_text()
                    st.code(content, language=f.suffix[1:] if f.suffix[1:] in ["yaml","json"] else "text")
                except Exception as e:
